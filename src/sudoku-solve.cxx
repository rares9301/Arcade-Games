...
bool checkBox(int i, int j, char n) {
    ...
    // search column for n
    for (int di = 0; di < 9; di++) {
        if (sudoku[di][j] == n) return false;
    }
    // search row for n
    for (int dj = 0; dj < 9; dj++) {
        if (sudoku[i][dj] == n) return false;
    }
    // search box for n
    for (int di = i - (i % 3); di < i - (i % 3) + 3; di++) {
        for (int dj = j - (j % 3); dj < j - (j % 3) + 3; dj++) {
            if (sudoku[di][dj] == n) return false;
        }
    }
    // n was not found
    return true;
...
int main() {
    cout << "[=] Please enter the sudoku matrix, line by line." << endl;
    ...
    cout << "[=] Press ENTER to start solving it." << endl;
    getch();
    cout << "\033[2J\033[1;1H"; // cls
    cout << "[+] Solving sudoku matrix..." << endl;
    // solve sudoku auto
    scanMatrix();
    cout << "[+] Finished solving the matrix." << endl;
